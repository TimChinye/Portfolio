...
const OrangeLogo = ({ className }: { className?: string }) => (
    <div className="text-orange-500">
        <BlackLogo className={className}/>
    </div>
);
...
export function NavbarLogo({
  ...
  const animatedFilter = useTransform(
    wipeProgress,
    [0, 100],
    // Light -> Dark ('top-down'): animate invert(0) to invert(1)
    // Dark -> Light ('bottom-up'): animate invert(1) to invert(0)
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{ filter: wipeDirection ? animatedFilter : idleFilter }}
      >
        <BlackLogo className="size-12" />
      </motion.div>
    </div>
  );
}