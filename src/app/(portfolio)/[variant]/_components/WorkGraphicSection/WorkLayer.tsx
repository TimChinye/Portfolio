...
// A placeholder for the Word component
// const Word = () => <h1 className="text-8xl font-bold text-black">WORK</h1>;
...
export function WorkLayer({ index, totalLayers, scrollYProgress, offsets }: WorkLayerProps) {
  const isLastLayer = index === totalLayers - 1;

  scrollYProgress = useTransform(scrollYProgress, (scrollY) => Math.min(scrollY, offsets[1]));

  // --- THE "GOLDEN TRIANGLE" OF ANIMATION CONTROLS ---

  // 3. How long does each word's individual (linear) animation last?
  const singleAnimationDuration = Math.pow(0.95, index);

  // 1. When does the FIRST word begin moving?
  const sequenceStartPoint = 0;

  // 2. When does the LAST word begin moving?
  const sequenceEndPoint = 0.875;

  // 4. The power of the easing curve for the cascade.
  ...
  // 1. Normalize the layer's position in the sequence (0 for the first, 1 for the last).
  // We use `totalLayers - 1` to ensure the final layer's index maps precisely to 1.0.
  const normalizedIndex = index / (totalLayers - 1);

  // 2. Apply the easing function to the normalized index. This is our quadratic curve.
  ...
  // 3. Calculate the total scroll duration available for the entire stagger sequence.
  const totalStaggerDuration = sequenceEndPoint - sequenceStartPoint;

  // 4. Map the eased progress to the actual scroll timeline to find THIS layer's start time.
  ...
  const y = useTransform(translateY, v => `${v}%`);
  // const y = useTransform(translateY, v => {
  //   let b = `calc(${v / 95} * (${clipperHeight}))`;
  //   console.log(b);
  //   return b;
  // });
