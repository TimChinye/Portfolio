...
-   **Unique Theme Switcher:** A custom "wipe" animation provides a visually engaging transition between light and dark modes, taking a "screenshot" of the old theme with `html2canvas-pro` and revealing the new one underneath. Logic is in `src/hooks/useThemeWipe.ts`.
